# Novel Reader 用户模块使用说明

**版本**: v1.2  
**更新时间**: 2026-02-18

---

## 📋 概述

用户模块已实现完整的用户认证、权限控制和收藏管理功能。

### 权限控制

| 用户类型 | 可访问页面 | 可操作功能 |
|---------|----------|----------|
| 未登录用户 | 小说浏览、小说详情 | 只能浏览，无法收藏 |
| 登录用户 | 小说浏览、小说详情、收藏管理、个人中心 | 浏览小说、收藏管理、修改个人信息 |
| 管理员 | 所有页面 | 全部功能 + 爬虫管理 |

---

## 🔐 接口列表

### 认证接口（4个）

#### 1. 用户注册
```
POST /api/auth/register
Content-Type: application/json

{
  "username": "testuser",
  "email": "test@example.com",
  "password": "Test@123"
}
```

**响应**：
```json
{
  "success": true,
  "message": "注册成功",
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
  "user": {
    "id": 1,
    "username": "testuser",
    "email": "test@example.com",
    "role": "USER"
  }
}
```

---

#### 2. 用户登录
```
POST /api/auth/login
Content-Type: application/json

{
  "email": "test@example.com",
  "password": "Test@123"
}
```

**响应**：
```json
{
  "success": true,
  "message": "登录成功",
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
  "user": {
    "id": 1,
    "username": "testuser",
    "email": "test@example.com",
    "role": "USER",
    "nickname": null,
    "avatarUrl": null
  }
}
```

---

#### 3. 获取当前用户信息
```
GET /api/auth/me
Authorization: Bearer <token>
```

**响应**：
```json
{
  "success": true,
  "user": {
    "id": 1,
    "username": "testuser",
    "email": "test@example.com",
    "role": "USER",
    "nickname": null,
    "avatarUrl": null
  }
}
```

---

#### 4. 用户注销
```
POST /api/auth/logout
Authorization: Bearer <token>
```

**响应**：
```json
{
  "success": true,
  "message": "注销成功"
}
```

---

### 用户接口（3个）

#### 5. 获取用户信息
```
GET /api/users/{id}
Authorization: Bearer <token>
```

**响应**：
```json
{
  "success": true,
  "user": {
    "id": 1,
    "username": "testuser",
    "email": "test@example.com",
    "nickname": "测试用户",
    "avatarUrl": "https://...",
    "role": "USER",
    "createdAt": "2026-02-18T10:00:00",
    "lastLoginTime": "2026-02-18T10:30:00"
  }
}
```

---

#### 6. 更新用户信息
```
PUT /api/users/profile
Authorization: Bearer <token>
Content-Type: application/json

{
  "nickname": "新昵称",
  "avatarUrl": "https://..."
}
```

**响应**：
```json
{
  "success": true,
  "message": "更新成功",
  "user": {
    "id": 1,
    "username": "testuser",
    "email": "test@example.com",
    "nickname": "新昵称",
    "avatarUrl": "https://...",
    "role": "USER"
  }
}
```

---

#### 7. 修改密码
```
PUT /api/users/password
Authorization: Bearer <token>
Content-Type: application/json

{
  "oldPassword": "Test@123",
  "newPassword": "NewPass@456"
}
```

**响应**：
```json
{
  "success": true,
  "message": "密码修改成功"
}
```

---

### 收藏接口（4个）

#### 8. 添加收藏
```
POST /api/favorites
Authorization: Bearer <token>
Content-Type: application/json

{
  "novelId": 1,
  "note": "很好看的小说"
}
```

**响应**：
```json
{
  "success": true,
  "message": "收藏成功",
  "favorite": {
    "id": 1,
    "novelId": 1,
    "platform": "ciweimao",
    "note": "很好看的小说",
    "hasUpdate": 0,
    "createdAt": "2026-02-18T10:00:00",
    "novel": {
      "title": "修仙从系统开始",
      "author": "逍遥子",
      "coverUrl": "https://...",
      "description": "..."
    }
  }
}
```

---

#### 9. 取消收藏
```
DELETE /api/favorites/{novelId}
Authorization: Bearer <token>
```

**响应**：
```json
{
  "success": true,
  "message": "取消成功"
}
```

---

#### 10. 获取收藏列表
```
GET /api/favorites?page=0&size=10
Authorization: Bearer <token>
```

**响应**：
```json
{
  "success": true,
  "content": [
    {
      "id": 1,
      "novelId": 1,
      "platform": "ciweimao",
      "note": "很好看的小说",
      "hasUpdate": 1,
      "novel": {
        "title": "修仙从系统开始",
        "author": "逍遥子",
        "coverUrl": "https://..."
      }
    }
  ],
  "totalElements": 100,
  "totalPages": 10,
  "size": 10,
  "number": 0
}
```

---

#### 11. 更新收藏备注
```
PUT /api/favorites/{novelId}/note
Authorization: Bearer <token>
Content-Type: application/json

{
  "note": "新的备注"
}
```

**响应**：
```json
{
  "success": true,
  "message": "更新成功",
  "favorite": {
    "id": 1,
    "note": "新的备注"
  }
}
```

---

### 分类接口（4个）

#### 12. 创建分类
```
POST /api/categories
Authorization: Bearer <token>
Content-Type: application/json

{
  "name": "玄幻小说",
  "description": "我喜欢的玄幻小说"
}
```

**响应**：
```json
{
  "success": true,
  "message": "创建成功",
  "category": {
    "id": 1,
    "name": "玄幻小说",
    "description": "我喜欢的玄幻小说",
    "sortOrder": 0
  }
}
```

---

#### 13. 获取分类列表
```
GET /api/categories
Authorization: Bearer <token>
```

**响应**：
```json
{
  "success": true,
  "categories": [
    {
      "id": 1,
      "name": "玄幻小说",
      "description": "我喜欢的玄幻小说",
      "sortOrder": 0,
      "createdAt": "2026-02-18T10:00:00"
    }
  ]
}
```

---

#### 14. 更新分类
```
PUT /api/categories/{id}
Authorization: Bearer <token>
Content-Type: application/json

{
  "name": "玄幻修仙",
  "description": "玄幻修仙类小说",
  "sortOrder": 1
}
```

**响应**：
```json
{
  "success": true,
  "message": "更新成功",
  "category": {
    "id": 1,
    "name": "玄幻修仙",
    "description": "玄幻修仙类小说",
    "sortOrder": 1
  }
}
```

---

#### 15. 删除分类
```
DELETE /api/categories/{id}
Authorization: Bearer <token>
```

**响应**：
```json
{
  "success": true,
  "message": "删除成功"
}
```

---

## 📊 权限说明

### 公开接口（无需认证）
- `POST /api/auth/register` - 用户注册
- `POST /api/auth/login` - 用户登录
- `GET /api/crawler/health` - 健康检查
- `GET /api/crawler/novels/**` - 小说浏览

### 用户接口（需要USER角色）
- `GET /api/auth/me` - 获取当前用户信息
- `POST /api/auth/logout` - 用户注销
- `GET /api/users/{id}` - 获取用户信息
- `PUT /api/users/profile` - 更新用户信息
- `PUT /api/users/password` - 修改密码
- `POST /api/favorites` - 添加收藏
- `DELETE /api/favorites/{novelId}` - 取消收藏
- `GET /api/favorites` - 获取收藏列表
- `PUT /api/favorites/{novelId}/note` - 更新收藏备注
- `POST /api/categories` - 创建分类
- `GET /api/categories` - 获取分类列表
- `PUT /api/categories/{id}` - 更新分类
- `DELETE /api/categories/{id}` - 删除分类

### 管理员接口（需要ADMIN角色）
- `GET /api/crawler/crawlers` - 爬虫列表
- `POST /api/crawler/trigger` - 触发所有爬虫
- `POST /api/crawler/trigger/{platform}` - 触发平台爬虫
- `GET /api/crawler/test/{platform}` - 测试爬虫
- `GET /api/crawler/status/{platform}` - 获取平台状态
- `PUT /api/crawler/configs/{id}` - 更新配置
- `GET /api/crawler/tasks` - 获取任务列表
- `GET /api/crawler/tasks/{id}` - 获取任务详情
- `POST /api/crawler/tasks` - 创建任务
- `PUT /api/crawler/tasks/{id}` - 更新任务

---

## 🔧 JWT使用说明

### Token格式
```
Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
```

### Token过期时间
- 默认：7天（604800000毫秒）
- 过期后需要重新登录

### Token包含的信息
```json
{
  "userId": 1,
  "username": "testuser",
  "role": "USER"
}
```

---

## ⚠️ 错误码

| 错误情况 | 返回信息 |
|---------|---------|
| 用户名已存在 | "用户名已存在" |
| 邮箱已被注册 | "邮箱已被注册" |
| 邮箱或密码错误 | "邮箱或密码错误" |
| 账号已被禁用 | "账号已被禁用" |
| 未登录 | "未登录" |
| 未收藏该小说 | "未收藏该小说" |
| 无权修改/删除 | "无权修改/删除该分类" |

---

## 📝 前端使用示例

### 登录示例
```javascript
import { crawlerApi } from './api'

// 登录
const loginData = {
  email: 'test@example.com',
  password: 'Test@123'
}

const result = await crawlerApi.login(loginData)
if (result.success) {
  // 保存Token到本地存储
  localStorage.setItem('token', result.token)
  localStorage.setItem('user', JSON.stringify(result.user))
}
```

### 添加收藏示例
```javascript
// 添加收藏
const favoriteData = {
  novelId: 1,
  note: '很好看的小说'
}

const result = await crawlerApi.addFavorite(favoriteData)
if (result.success) {
  console.log('收藏成功')
}
```

### 获取收藏列表示例
```javascript
// 获取收藏列表
const result = await crawlerApi.getFavorites({ page: 0, size: 10 })
if (result.success) {
  console.log('收藏列表：', result.content)
  console.log('总数：', result.totalElements)
}
```

---

## 🎯 前端路由守卫示例

```javascript
import { createRouter, createWebHistory } from 'vue-router'

const routes = [
  {
    path: '/novels',
    component: Novels,
    meta: { requiresAuth: false }  // 公开页面
  },
  {
    path: '/favorites',
    component: Favorites,
    meta: { requiresAuth: true, role: 'USER' }  // 需要登录
  },
  {
    path: '/admin/crawler',
    component: CrawlerAdmin,
    meta: { requiresAuth: true, role: 'ADMIN' }  // 需要管理员
  }
]

const router = createRouter({
  history: createWebHistory(),
  routes
})

// 路由守卫
router.beforeEach((to, from, next) => {
  const token = localStorage.getItem('token')
  const user = JSON.parse(localStorage.getItem('user') || '{}')

  if (to.meta.requiresAuth) {
    if (!token) {
      // 未登录，跳转到登录页
      next('/login')
      return
    }

    // 检查角色权限
    if (to.meta.role && user.role !== to.meta.role) {
      // 权限不足
      next('/')
      return
    }
  }

  next()
})

export default router
```

---

## 📞 常见问题

### Q1: 如何创建管理员账户？
**A**: 目前需要手动在数据库中设置用户的role为"ADMIN"。后续可以添加管理员创建接口。

### Q2: Token过期后怎么办？
**A**: Token过期后，前端需要拦截401错误，跳转到登录页面，用户重新登录获取新Token。

### Q3: 如何在请求中携带Token？
**A**: 在请求头中添加：`Authorization: Bearer <token>`

### Q4: 权限不足会返回什么？
**A**: 返回403 Forbidden错误，前端需要提示用户权限不足。

### Q5: 可以同时拥有多个角色吗？
**A**: 当前设计每个用户只有一个角色（USER或ADMIN），可以根据需要扩展为多角色。

---

**文档创建时间**: 2026-02-18  
**版本**: v1.2  
**状态**: 后端已完成，前端待完善  

🦞
